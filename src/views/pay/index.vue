...
<script>
import { requestGetAddressList } from '@/api/address'
import { requestCheckOrder, requestSubmitOrder } from '@/api/order'
export default {
  name: 'PayIndex',
  data () {
    return {
  ...
  methods: {
    async getAddressList () {
      const { data: { list } } = await requestGetAddressList()
    ...
    async getOrderList () {
      // 购物车结算
      if (this.mode === 'cart') {
        const { data: { order, personal } } = await requestCheckOrder(this.mode, { cartIds: this.cartIds })
        this.order = order
        this.personal = personal
      }
      // 立即购买
      if (this.mode === 'buyNow') {
        const { data: { order, personal } } = await requestCheckOrder(this.mode, {
    ...
    async submitOrder () {
      if (this.mode === 'cart') {
        await requestSubmitOrder(this.mode, {
