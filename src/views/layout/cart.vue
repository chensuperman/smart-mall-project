<template>
  ...
  <div v-if="isLogin&&cartList.length>0">
    ...
    <div class="cart-list">
      <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
        ...
        <div class="info">
          <span  @click="$router.push(`/prodetail/${item.goods_id}`)" class="tit text-ellipsis-2">{{item.goods.goods_name}}</span>
          <span class="bottom">
            <div class="price">¥ <span>{{item.goods.goods_price_min}}</span></div>
            <!-- 既希望保留原本的形参，又需要通过调用函数传参=>箭头函数包裹一层 -->
  ...
  <div class="empty-cart" v-else>
    ...
    <div class="footer-fixed">
      <div @click="totalAllCheck" class="all-check">
...
export default {
  ...
  methods: {
    toggleChecked (goodsId) {
      this.$store.commit('cart/toggleAllCheck', goodsId)
    },
    totalAllCheck () {
      this.$store.commit('cart/totalAllCheck', !this.isAllChecked)
    },
  ...
  watch: {
    ...
    // 监听isEdit,让进入编辑模式时全选取消，结算时全选
    isEdit (value) {
      if (value) {
        this.$store.commit('cart/totalAllCheck', false)
      } else {
        this.$store.commit('cart/totalAllCheck', true)
      }
    }
  }
}
